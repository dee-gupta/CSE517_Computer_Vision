...
#face_cascade = cv2.CascadeClassifier('/usr/local/share/OpenCV/#haarcascadeshaarcascade_frontalface_default.xml')
...
def mycamshift(v, file_name):
    ...
    outvid = cv2.VideoWriter(output_name+'.mp4', fourcc, 20.0, (int(v.get(3)),int(v.get(4))))
    ...
    while(True):
        ...
        cv2.circle(frame,(int(pt_x),int(pt_y)), 2, (0,0,255), -1)
        outvid.write(frame)
...
def myparticle(v, file_name):
    ...
    while(True):
        ...
        for xy in particles:
            cv2.circle(frame,(xy[0],xy[1]), 1, (0,0,255), -1)
...
def myof(v, file_name):
    ...
    mask = np.zeros((prev_frame.shape[0], prev_frame.shape[1]), np.uint8)
    mask[pt_y - 30 : pt_y + 20, pt_x - 30 : pt_x + 20] = 255

    prev_pts = cv2.goodFeaturesToTrack(prev_frame, mask = mask, **feature_params)
    # prev_pts = np.array([[[pt_x, pt_y]]], dtype='float32')
    ...
    while(1):
        ...
        # getting avg of point if measurement isn't valid
        if not measurement_valid:
            x, y = 0, 0
            for pt in next_pts:
                x += pt[0]
                y += pt[1]
            pt_x, pt_y = int(x) / len(next_pts), int(y) / len(next_pts)
