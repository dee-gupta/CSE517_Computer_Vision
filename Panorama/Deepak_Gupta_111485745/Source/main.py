...
def Laplacian_Pyramid_Blending_with_mask(A, B, m, num_levels = 4):
...
def Perspective_warping(img1, img2, img3):
    ...
    #warp image 2 in the perspective of image 1
    img1 = cv2.warpPerspective(img2, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_TRANSPARENT)

    (M, pts1, pts2, mask) = getTransform(img3, img1, 'homography')
    
    # warp image 3 in the perspective of image 1 
    img1 = cv2.warpPerspective(img3, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_TRANSPARENT)
    output_image = img1
...
def Bonus_perspective_warping(img1, img2, img3):
    ...
    out_mask = cv2.warpPerspective(np.ones_like(img2), M, (img1.shape[1],img1.shape[0]), dst=np.zeros_like(img1), borderMode=cv2.BORDER_TRANSPARENT)
    #use pyramid blending with mask
    img1 = np.uint8(Laplacian_Pyramid_Blending_with_mask(out, img1, np.float32(out_mask), num_levels=4))

    (M, pts1, pts2, mask) = getTransform(img3, img1, 'homography')
    # then transform im1 with the 3x3 transformation matrix
    out = cv2.warpPerspective(img3, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpPerspective(np.ones_like(img3), M, (img1.shape[1],img1.shape[0]), dst=np.zeros_like(img1), borderMode=cv2.BORDER_TRANSPARENT)
    img1 = np.uint8(Laplacian_Pyramid_Blending_with_mask(out, img1, np.float32(out_mask), num_levels=4))
...
def Cylindrical_warping(img1, img2, img3):
    ...
    im1 = cv2.copyMakeBorder(imcyl1[0],50,50,300,300, cv2.BORDER_CONSTANT)
    (M, pts1, pts2, mask) = getTransform(imcyl2[0], im1)
    out = cv2.warpAffine(imcyl2[0], M[:-1], (im1.shape[1],im1.shape[0]), dst=im1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpAffine(imcyl2[1], M[:-1], (im1.shape[1],im1.shape[0]), dst=np.zeros_like(im1), borderMode=cv2.BORDER_TRANSPARENT)/255
    im1 = out*(out_mask)+im1*(1-(out_mask))
    
    (M, pts1, pts2, mask) = getTransform(imcyl3[0], im1)
    # then transform im1 with the 3x3 transformation matrix
    out = cv2.warpAffine(imcyl3[0], M[:-1], (im1.shape[1],im1.shape[0]), dst=im1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpAffine(imcyl3[1], M[:-1], (im1.shape[1],im1.shape[0]), dst=np.zeros_like(im1), borderMode=cv2.BORDER_TRANSPARENT)

    im1 = out*(out_mask/255)+im1*(1-(out_mask/255))
...
def Bonus_cylindrical_warping(img1, img2, img3):
    ...
    (M, pts1, pts2, mask) = getTransform(imcyl2[0], im1)
    # then transform im1 with the 3x3 transformation matrix
    out = cv2.warpAffine(imcyl2[0], M[:-1], (im1.shape[1],im1.shape[0]), dst=np.zeros_like(im1), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpAffine(imcyl2[1], M[:-1], (im1.shape[1],im1.shape[0]), dst=np.zeros_like(im1), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.GaussianBlur(out_mask,(5,5),0)
    im1 = out*(out_mask/255)+im1*(1-(out_mask/255))
    im1 = np.uint8(Laplacian_Pyramid_Blending_with_mask(out, im1, np.float32(out_mask/255), num_levels=3))

    (M, pts1, pts2, mask) = getTransform(imcyl3[0], im1)
    # then transform im1 with the 3x3 transformation matrix
    out = cv2.warpAffine(imcyl3[0], M[:-1], (im1.shape[1],im1.shape[0]), dst=np.zeros_like(im1), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpAffine(imcyl3[1], M[:-1], (im1.shape[1],im1.shape[0]), dst=np.zeros_like(im1), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.GaussianBlur(out_mask,(5,5),0)
    im1 = out*(out_mask/255)+im1*(1-(out_mask/255))
    im1 = np.uint8(Laplacian_Pyramid_Blending_with_mask(out, im1, np.float32(out_mask/255), num_levels=3))
