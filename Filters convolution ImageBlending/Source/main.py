...
import cv2
import numpy
from histogram import getEqualizeImage
from filters import getLowPassImage
from filters import getHighPassImage
from deconvolution import getDeconvImage
from pyramid_blend import pyrBlend
...
def histogram_equalization(img_in):
   ...
   img_out = getEqualizeImage(img_in) # Histogram equalization result 
...
def low_pass_filter(img_in):
   ...
   img_out = getLowPassImage(img_in) # Low pass filter result
...
def high_pass_filter(img_in):
   ...
   img_out = getHighPassImage(img_in) # High pass filter result
...
def deconvolution(img_in):
   ...
   img_out = getDeconvImage(img_in) # Deconvolution result
...
def laplacian_pyramid_blending(img_in1, img_in2):
   ...
   img_out = pyrBlend(img_in1, img_in2) # Blending result
